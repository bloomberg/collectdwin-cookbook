...
module CollectdWinCookbook
  module Resource
    ...
    class CollectdWinConfig < Chef::Resource
      ...
      attribute(:directory, kind_of: String, default: "#{ENV['ProgramW6432']}/CollectdWin/config")
      ...
      def content
        ...
        hdr + write_elements(configuration).concat("\n")
      end
      ...
      def write_elements(directives, indent = 0)
        ...
        directives.dup.map do |key, value|
          ...
          if value.is_a?(Array)
            ...
            if value.length <= 0 || (value.length == 1 && value.has_key?('attr'))
              %(#{tabs}<#{key}#{attrstr}/>)
            else
              [%(#{tabs}<#{key}#{attrstr}>),
